...
$inicio = ($pag*$qnt) - $qnt;
$db->conecta();	
$db->query( "select * from noticias WHERE not_view = 's' order by not_date desc, not_time desc LIMIT ".$inicio.",".$qnt." ;" )->fetchAll();
    if ( $db->rows >= 1 )
    {
        $nots = $db->data;
					...
					<div class="paging-nav col-xs-12">
						<span class="float-left">    
                      	<?php
							...
							$sql_all = "select * from noticias WHERE not_view = 's' order by not_date desc, not_time desc";
