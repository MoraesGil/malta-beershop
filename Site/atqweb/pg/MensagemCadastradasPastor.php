<p>Você está em -> <a href="Home">Home </a>-> <a href="MensagemCadastradasPastor">Mensagens Cadastradas Pastor</a></p>
...
e.usuario_id = u.id 
WHERE mensagem_titulo LIKE '%".$pesquisar."%'  ORDER BY mensagem_dataoriginal DESC;")->fetchAll();
} else {
$db->query("SELECT * FROM mensagem e
INNER JOIN usuarios u ON
e.usuario_id = u.id 
WHERE usuario_id = '".$usuario->getId()." AND 'mensagem_titulo LIKE '%".$pesquisar."%'  ORDER BY mensagem_dataoriginal DESC;")->fetchAll();
}
    if ( $db->rows == 0 )
		{ 
...
?>
             ...
             <tr>
                ...
                <td>  <?php echo $ln->nome; ?>	     </td>
                  <td><a href="AtualizarMensagensPastor&id=<?php echo $ln->mensagem_id; ?>" title="Editar"><button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-pencil"></span></button></a></td>
...
$db->conecta();	
			?>
    <h2 class="sub-header">Mensagens Cadastradas Convidado</h2> 
          <div class="table-responsive">
            <table class="table table-striped">
              <thead>
                <tr>
...
<?php
        if ($usuario->getTipo() == "ADMIN") {   
				...
				echo ' <li><a href="MensagemCadastradasPastor&pag=1" target="_self" title="Primeira">&laquo;</a></li>';
				for($i = $pag-$max_links; $i <= $pag-1; $i++) {
				 if($i <=0) { 
				} else { 
				echo ' <li ><a href="MensagemCadastradasPastor&pag='.$i.'" target="_self">'.$i.'</a></li>';
				} } 
				 echo '<li ><a href="MensagemCadastradasPastor&pag='.$i.'" target="_self">'.$pag.'</a></li>'; 
				 for($i = $pag+1; $i <= $pag+$max_links; $i++) { 
				 if($i > $pags) { 
				 } 
				 else { echo '<li><a href="MensagemCadastradasPastor&pag='.$i.'" target="_self">'.$i.'</a></li>'; } }
				echo '<li><a href="MensagemCadastradasPastor&pag='.$pags.'" target="_self" title="Última">&raquo;</a></li>	';
