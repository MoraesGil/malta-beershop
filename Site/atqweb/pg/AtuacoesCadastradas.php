...
$db->conecta();
//inicio da query
$query = "SELECT a.id, a.nome, u.nome as usuario  FROM atuacoes a
...
<div class="table-responsive <?php echo $conteudo=="" ? $conteudo : "hidden" ?>" >
			...
			{ 	// linhas pesquisa
				...
				{// loop conteudo pesquisa
					...
					<tr>
						...
						<td>
							<a href="AtualizarAtuacao&id=<?php echo $obj->id; ?>" title="Editar"><button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-pencil"></span></button></a>
						</td>
						<td>
							<a href="#" onclick="javascript: if (confirm('Você realmente deseja excluir?'))location.href='ExcluirAtuacao&id=<?php echo $obj->id; ?>'" title="Excluir">
	...
	if($conteudo=="")//tem registros entao..
	{
		...
		<nav>
			<ul class="pagination">
				<li>
					<a href="AtuacoesCadastradas&p=1" aria-label="Primeira" title="Primeira">
				...
				for ($i = $pag-2; $i <= $pag+2; $i++){
					if($i>0 && $i<=$pags)
					{	echo '<li '.($i==$pag ? ' class="active"':'').'><a href="AtuacoesCadastradas&pag='.$i.'" target="_self" >'.$i.'</a></li>';}
				}
				?>
				<li>
					<a href="AtuacoesCadastradas&pag=<?php echo $pags?>" aria-label="Última" title="Última">
