<p>Você está em -> <a href="Home">Home </a>-> <a href="HotSitePedidosCadastrados">HOT SITE - Pedidos Cadastrados</a></p>
...
?>
              ...
              <tr>
                  ...
                  <td><a href="HotSiteAtualizarPedidos&id=<?php echo $ln->hotsite_pedid; ?>" title="Editar"><button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-pencil"></span></button></a></td>
                  <td><a href="#" onclick="javascript: if (confirm('Você realmente deseja excluir?'))location.href='HotSiteExcluirPedidos&id=<?php echo $ln->hotsite_pedid; ?>'" title="Excluir">
...
?>
            ...
            <p align="center"><a href="HotSitePedidosCadastrados" title="Voltar"> Voltar </a></p>
          </div>
<?php
$db->fechaConexao();
$db->conecta();	
...
$db->conecta();	
			?>
	<h2 class="sub-header">Pedidos de Oração Aguardando Aprovação</h2> 
          <div class="table-responsive">
            <table class="table table-striped">
              <thead>
                <tr>
...
$db->query("SELECT * FROM hotsite_pedidos  h
	...
	$db->query("SELECT * FROM hotsite_pedidos  h
			...
			h.usuario_id = u.id
				...
				if ( $db->rows == 0 ){
					echo "<p class='moderar' align='center'>Não contém nenhum pedido de oração para ser aprovado!</p>";
				}else
				{ // linhas
     			foreach ( $db->data as $linhas )
       			{// loop conteudo
...
?>
  			  <tr>
            	  ...
            	  <td><?php echo $funcoes->formataDateTime($ln->hotsite_peddatetime);?></td>
            	  <td><a href="#" onclick="javascript: if (confirm('Você realmente deseja aprovar o pedido de oração'))location.href='HotSiteAprovarPedidos&id=<?php echo $ln->hotsite_pedid; ?>&idusuario=<?php echo $usuario->getId(); ?>'" title="Aprovar">
...
?>             
    ...
    <h2 class="sub-header">Pedidos de Oração Postados</h2> 
          <div class="table-responsive">
            <table class="table table-striped">
              <thead>
                <tr>
...
<?php
				...
				if ($usuario->getTipo()== 'ADMIN'){
			$sql_all = "SELECT * FROM hotsite_pedidos  h
			LEFT JOIN usuarios u ON
			h.usuario_id = u.id
			ORDER BY hotsite_peddatetime DESC;";
	}
else
	{
			...
			h.usuario_id = u.id
				...
				echo '<a href="HotSitePedidosCadastrados&p=1" target="_self" title="Primeira"><button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-chevron-left"></span></button></a> '; 
				for($i = $pag-$max_links; $i <= $pag-1; $i++) {
				 if($i <=0) { 
				} else { echo '<a href="HotSitePedidosCadastrados&p='.$i.'" target="_self"><button type="button" class="btn btn-default btn-xs">'.$i.'</button></a> '; } } 
				 echo '<span> <button type="button" class="btn btn-default btn-xs"><strong>'.$pag.'</strong></button>'; 
				 for($i = $pag+1; $i <= $pag+$max_links; $i++) { 
				 if($i > $pags) { 
				 } 
				 else { echo '<a href="HotSitePedidosCadastrados&p='.$i.'" target="_self"><button type="button" class="btn btn-default btn-xs">'.$i.'</button></a> '; } }
				 echo '<a href="HotSitePedidosCadastrados&p='.$pags.'" target="_self" title="�ltima"><button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-chevron-right"></span></button></a> ';
